...
- **stemming**: using the root word/noun/verb instead on the particular *inflection* du jour used here in text *X*: stemming *generalizes* the source and query texts.
...
#### And we must talk about stemming, my dear

**Stemming** is the algorithm used to produce -- for both indexing and searching -- the root word/noun/verb, *replacing* the various particular *inflections* used in texts *X*, *Y* and *Z*: *stemming* **generalizes** the source such that *single/plural* in nouns is made irrelevant, thus ensuring the lines/documents carrying only one of *either* form will be *hit* and consequently end up listed in the search results for your perusal: this is what you, as the user, wanted, with high probability, and thus *stemming* is often considered another quality improvement.

f.e.: you did not care when looking for "`test trials`" whether an author wrote a case paper about a single "`test trial`" or a review paper discussing multiple "`test trials`": you'll get to see both when *stemming* is applied as both stem to the root "`test trial`".

Ditto for texts written in *present/future/past(-perfect) tense*: while English and several other Indo-European-languages use additional "assistant" verb words ("`would/will/have/had/...`") to indicate a particular *tense*, plenty languages (Latin, Italian, Finnish??, Russian, etc.) use *inflection of the root* word (the verb itself) and thus will NOT be hit when you write your search query... unless you apply stemming to your verbs. English is a bit of a mixed bag here; Latin is one example where verbs are inflected with gusto. (And I haven't even addressed Asian languages' inflection rules yet. What joy!)

f.e. consider the English "`will go home`" vs. "`went home`" vs. "`has gone home`" vs. "`is going home`": all of these SHOULD be marked as a *hit* when you query *in any way* about some activity "`[to] go home`", shouldn't they? Another scenario where stemming helps to improve your search results. 

Also consider mixed modes where words, e.g., "`ranking`" and "`[search] rank`" are to be considered on par: both nouns represent the same root concept: `[search] rank`, while the first of these might be employed as a verb instead of a noun, as in "`we are ranking these results`" vs. "`we apply a ranking to these documents`". Of course, *stemming* is highly language-specific, hence *automatic language identification* is implied as yet another preprocessing block during both training and query (pre-)processing!


##### Advanced stemming-alike preprocessing

When you follow through on the concept of stemming as a positive addition to your search quality improvement efforts, the obvious next bit we cannot accomplish with (automated) stemming algorithms: generalizing through treating synonyms, pseudonyms, etc. as equivalent to some root concept!
Enter [Stanford's GloVe: Global Vectors for Word Representation](https://nlp.stanford.edu/projects/glove/), which is one major attempt to answer this question. As far as I can tell, they only did this for the American(?) English language, which' resulting synonyms' mapping models can be downloaded from the Net. 

I have not been able to find any similar, publicly announced & accessible efforts for any other human language, so here surely [*you have got your work cut out for you*](https://www.ldoceonline.com/dictionary/have-your-work-cut-out-for-you).

And [*you got that in one*](https://www.ldoceonline.com/dictionary/got-it-in-one): GloVe DOES NOT address **idiom**, which is extremely language-specific and often localized in aspects: time, (geo-)location and/or [*métier*](https://www.ldoceonline.com/dictionary/metier) where *idiom* is often identified as (part of the professional) *jargon*.
> Also note that *idiom* is the last part of language for non-native human speakers to learn and use, i.e. to acquire properly. To the humans around you, it's one of the subtle life-long indicators of your *savvy*, your *upbringing*, your *otherness*: are you *one of us*, or aren't you? Hm, where are you *from*?! *Just asking*...[^1]


[^1]:: To answer: I am Eastern *Dutch* -- yeah, that's *a thing*, which, *most empathetically*, is not to be confused with *Holland*, though both are part of *The Netherlands*, and *all of 'em* are utterly indistinguishable from a fly speck on any reasonably priced globe, *so what's the bother with that Eastern bit of emphasis anyhow, eh, guv'*? Indeed, right you are. *Anyhoo*, English is NOT my mother tongue. While I have been accused by a few individuals as having good language skills and even "a way with words" once, I am very much aware that I am, and always will be, until my dying day, an *alien*, for I wasn't born into the language. *My tell*? I mix ['murican](https://en.wikipedia.org/wiki/American_English) with [British](https://simple.wikipedia.org/wiki/British_English) and other bits of [Ye Olde Empire](https://en.wikipedia.org/wiki/Dominion) without compunction. [Therefore, I must be a bloody foreigner!](https://www.pinsentmasons.com/out-law/news/bloody-foreigners-is-racist-taunt-say-lords)



> With utmost regret, I note that the bulk of today's research' finest are focused on *vision* AI (license plates, identifying and tracking individuals and other components of society, large and small: cars, bikes, packages/mail, ...) as that's where the *cheddar* is *at*, while text treatment (for search engines) is much less vector data intensive, yet far from easy, let alone being "*a solved problem*" as you might have already gleaned from the conundrums mentioned above. Alas.
> 
> 
...
To help my confusion, it took me a while to realize that some experts file *stemming* under the general chapter of *normalization*, while a few interesting others don't. I am certainly far from expert in this field, but, while I do understand *why* one would categorize the **stemming** *genus* as just another member of the overarching **normalization** *clade*, personal preference is to treat *stemming* as a separate subject matter of the same importance as *normalization*, as used above: **normalization** is, to me, about automated *correction of technical data errors*, such as encodings, text recognition post cleanup to compensate for OCR engine imperfections and human data entry mistakes, which may be approached, f.e., through automated spelling correction. 
As we all know from our own modern use of mobile phones' chat and mail writing in-built autocorrect features, this is already a dangerously inaccurate chunk of machinery: how often did you notice a grave autocorrect-induced *eff up* in messages already sent?! I plead guilty to the charge. In this regard, regrettably, I am a recidivist. And I bet you are too!

That automated correction activity is all in the zone of **technical normalization**, at least arguably more or less as *it all starts with an error, either by human or machine, early in the data sourcing path* of the process (both training and querying: you can also type a typo in your search query and have it fatally autocorrected, or not: either way, you have injected a data source flaw near the start of either process!

My argument is that **stemming** et al (**including its advanced extensions**, GloVe being one of those, to my mind) is another complex subject to deal with, with lots of open research questions, which **is concerned with *generalizing* the subject matter you want to be discoverable**, i.e. being processed into your search index or search query!
Also, **stemming is NOT part of the *filtering* category either**, as ***filtering*** generally includes approaches such as downgrading or entirely *removing stopwords* from the source data streams: *stopwords* et al **are approaches aimed at improving practical limitations such as improving the delivery response times** (which would otherwise be prohibitively slow and thus unacceptable low grade usability) **and search index storage space**: $O(log (N))$ and other $O(better\_than\_linear \simeq O(N))$ algorithms are great for large scale data sets' operations, but without judicious filtering your search indexes'$N$ simply goes through the roof, as $N$ at this machine component level is *the number of different n-grams*, which is not simply equal to the number of words in your source texts' vocabulary, but rather a multiple thereof and **the aim of *filtering* using stopwords and other means is to reduce the total vocabulary set at n-gram level to a manageable count $N$** as the reality is that even if everything we do were $O(1)$ algorithmically -- which would be *fantastic*, in both senses -- then we must realize that those $O(\ldots)$ CS theoretical performance expressions are obscuring some pretty impactful practical performance obstructions underneath: with $N$ sufficiently *small*, $N$ might be -- *ideally* -- cached in your CPU's cache memory whence $O(1)$ reads in a CPU clock cycle multiple per $1$ operation. Run the otherwise identical system at a slightly *larger* $N$ and your $1$ now must be read in the number of RAM bus accesses: CPU RAM bus access on all modern systems is (much) slower than CPU cache memory access, so whatever your $O(\ldots)$ is, it's already slowing down. This is the zone where you hear things like "the database is loaded into memory; when you add data, you'll need a mainboard with more RAM!" from your database experts and other folk. Increase that $N$ of yours some more, not by a lot, and you end up facing the next performance disaster: it's not just cache misses any more, you will be experiencing RAM page faults by the dozen as *all* your data could not be loaded into available RAM any more: now your $1$ is a mix of *hard disc accesses* and RAM bus access operations. *Disc access* is much slower than RAM bus access so your *optimal* algorithm will slow down again, and probably by a lot! Keep increasing that $N$ and the disc access pattern will start to worsen in such a way that now that theoretical $1$ is expressed not merely in already slow disc sector read accesses, but now is to be expressed in the  number of *disc track/sector seek operations* as the hard disc(s) must move their heads around to fetch the data du jour: with state of the art hardware, the conclusion is that theoretical $O(1)$ -- or *any* other $O(\ldots)$ expression -- can consume from about 1 clock tick (say a *quarter of a nanosecond*) up to *several milliseconds*, as that's what a disc seek command takes for even the latest & fastest large capacity spinning metal hard discs' storage components, ergo: we're looking at a multiplier ratio of about 1:1000000, depending on the amount of data you want to store, which is your $N$ for the theorists. And please *do realize* this ratio is applicable *all over*, no matter what algorithmic heady stuff you come up with!  Consequently, *filtering* is the (hacky) answer to reduce your total $N$, balancing search/indexing power and query response quality with throwing away and otherwise ignoring as much as you can when building the mandatory search indexes. Of course, for the dedicated AI professional, the alternative answer is: buy more hardware & lots of it! Which easily explains why Nvidia has been reporting such astronomic figures over the years: their chips power the serious hardware.

Qiqqa is not Google, Qiqqa is intended to be available "*off line*" and on your own private and business hardware; your data privacy trumps search quality and search response speed, so Qiqqa has to make some very tough choices, which, by necessity, will be very different from what the big players do, report or advise in their happy news "*look ma! no hands!*" publications. *Filtering* has a very prominent role to play here, while *normalization* acts as a data cleaning phase, which should help getting the results you ask for as it improves the source data quality for the same $N$ (less noise pollution in your source data), and *generalization* (e.g. stemming) is there to help produce potentially higher quality tokens (n-grams and whatnot) at a hopefully reduced vocabulary size, thus being the first, if minor, step towards max size ($N$) index reduction. *Filtering*, however, is where the game is at, when it comes to search index size reduction per unit of source text content.
