...
- Furthermore it's a **bad idea** to start any filename with `"-"` or `"--"` as this clashes very badly with standard UNIX commandline options; only a few tools "fix" this by allowing a special `"--"` commandline option, e.g. 
