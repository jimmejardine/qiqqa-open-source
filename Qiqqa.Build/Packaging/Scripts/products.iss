...
en.depdownload_admin=An Administrator account is required to install some of these Microsoft products that Qiqqa depends on.%nPlease run this setup again as an Administrator (or using 'Run as Administrator') or install the following products manually:%n%n%1%nClose this message and press Cancel to exit setup.
