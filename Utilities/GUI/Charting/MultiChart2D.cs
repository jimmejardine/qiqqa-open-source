...
namespace Utilities.GUI.Charting
{
	...
	{
		static Color color_almost_white = Color.FromArgb(248,248,248);

		public float title_height = 60;
		public float legend_height = 60;
		public float axis_height = 60;

		double axis_tick_gap_min = 50;
		double legend_gap_width = 16;
		double legend_indicator_size = 10;
		double legend_indicator_gap_width = 5;
		SolidBrush brush_background = new SolidBrush(Color.White);
		SolidBrush brush_background_axisguide = new SolidBrush(color_almost_white);
		SolidBrush brush_series = new SolidBrush(Color.Black);
		Pen pen_border = new Pen(Color.Black);
		Pen pen_series = new Pen(Color.Black, 1.0f);
		Pen pen_axis = new Pen(Color.Black);
		Pen pen_axis_guide = new Pen(color_almost_white, 2.0f);
		float point_size = 6.0f;
		float axis_notch_height = 5.0f;
		Font font_title = new Font(FontFamily.GenericSansSerif, 16);
		Font font_axis = new Font(FontFamily.GenericSansSerif, 13);
        Font font_legend = new Font(FontFamily.GenericSansSerif, 14);
		SolidBrush brush_axis = new SolidBrush(Color.Black);
		SolidBrush brush_legend = new SolidBrush(Color.Black);
		Pen pen_legend = new Pen(Color.Black);
		SolidBrush brush_title = new SolidBrush(Color.Black);
...
//			doSampleChart();
		...
		{
            if (!suspendingRefresh) this.Refresh();
		}
		...
		void getSeriesExtents(out Point2D min_pri, out Point2D max_pri, out Point2D min_sec, out Point2D max_sec)
		{
		...
		bool testAndDivide(int divisor, ref int tick_divisor)
		{
		...
		bool haveSeriesForChartAxis(ChartAxis chartaxis)
		{
			for (int i = 0; i < series_list.Count; ++i)
			{
				Series series = (Series) series_list[i];
		...
		int determineTickDivisor(double y_extent, double axis_tick_gap_min)
		{
		...
		DashStyle pickSeriesDashStyle(int i, ChartType charttype)
		{
		...
		void renderSeriesLegend(int i, Series series, Graphics g, Pen pen_series, Brush brush_series, float x_pos, float y_pos, float point_size)
		{
		...
		void renderSeriesPoint(int i, Graphics g, Pen pen_series, Brush brush_series, float x_pos, float y_pos, float point_size)
		{
			switch (i % 5)
			{
				case 0:
        ...
        {
            base.SuspendLayout();
            this.suspendingRefresh = true;
        }

        public void ResumeRefresh()
        {
            base.ResumeLayout();
            this.suspendingRefresh = false;
            this.Refresh();
        }
        ...
        private void InitializeComponent()
        {
            ...
            // MultiChart2D
            // 
            this.Name = "MultiChart2D";
            this.Size = new Size(450, 368);
