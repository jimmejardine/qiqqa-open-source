// -------------------------------------------------------------
// --- © Copyright 2011 Quantisle Ltd.  All Rights Reserved. ---
// -------------------------------------------------------------
...
Sys.prototype.getAbbreviations = function (name, vartype) {
    Log("Sys.getAbbreviations called");
...
var QiqqaIncite = function () {
    Log("Starting InCite");

    Log("Creating Sys");
    var sys = new Sys();

    Log("Creating CSL.Engine");
    var citeproc = new CSL.Engine(sys, CITATION_STYLE);

    Log("Setting output format");
    citeproc.setOutputFormat("rtf");

    Log("Updating items");
    citeproc.updateItems(CITATION_INIT);

    Log("Generating inlines");
    var json_inlines = [];
    for (i = 0; i < CITATION_USES.length; ++i) {
        var citation = CITATION_USES[i];
        var citation_output = citeproc.appendCitationCluster(citation, false);
        json_inlines.push(
        {
            "citation": citation,
    ...
    Log("Generating bibliography");
    var json_bibliography = citeproc.makeBibliography();

    Log("Setting text elements");
    var json_output =
    {
        "inlines": json_inlines,
        "bibliography": json_bibliography,
        "logs": logs
    };
    $("#ObjOutput").text(JSON.stringify(json_output));

    // We throw this error so that .NET can be notified that we are done!  <sigh>
    INCITE_FINISHED();
};
