...
const pdf_list = fs.readFileSync(opts._[0], "utf8").split('\n').map(f => f.trim().replace(/^"\.\//, '').replace(/"$/, '')).filter(f => f.length > 2);
...
---------------------------------------------
   ...
   for (let i = 1; i < opts._.length; i++) {
      ...
      // Now seek out every DataRow and match it against it against our list of PDF files:
      ...
      cs_content = cs_content.replace(chunk_re, (m, p1, p2, p3) => {
         let cc = snippet.replace("XXXXXXXXX", `Test_PDF_bulk_chunk${ counter.toString(10).padStart(4, '0') }`);
         counter++;
